...
class PostViewSet(viewsets.ModelViewSet):
    ...
    search_fields = ['title', 'content',
                     'author__username', 'category__name', 'tags__name']

    def get_queryset(self):
        return Post.objects.select_related('author', 'category') \
            .prefetch_related('tags', 'comments__replies', 'likes')
    ...
    def related(self, request, pk=None):
        ...
        if not query:
            related_posts = self.get_queryset().exclude(
                id=post.id).order_by('-created_at')[:3]
        else:
            related_posts = self.get_queryset().filter(query).exclude(
                id=post.id).distinct().order_by('-created_at')[:3]
