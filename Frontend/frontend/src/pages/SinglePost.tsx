...
import { useQuery, useQueryClient } from "@tanstack/react-query"; // <-- IMPORTED REACT QUERY
...
const SinglePost: React.FC = () => {
  ...
  // 1. Initialize the Query Client to tap into the cache
  const queryClient = useQueryClient();

  // --- THE MAGIC: INSTANT CACHE INJECTION ---
  const { data: post, isLoading } = useQuery<PostDetail>({
    ...
    initialData: () => {
      // Look inside the 'posts' array we already downloaded for the homepage
      const allPosts = queryClient.getQueryData<PostDetail[]>(["posts"]);
      
      // Find the exact post the user just clicked
      const cachedPost = allPosts?.find((p) => String(p.id) === String(id));

      // If we found it, return it instantly! (Skips the loading spinner)
      if (cachedPost) {
        return cachedPost;
      }
      return undefined;
  ...
  // Data States (Only for comments now)
  ...
  // 2. Sync Likes when the background fetch completes
  useEffect(() => {
    if (post && post.likes) {
  ...
  // 3. Fetch Comments independently
  useEffect(() => {
    if (post) {
      ...
      <article className="pb-5 mb-5 mt-5 container">
        <div className="article-container placeholder-glow">
          ...
          <div className="d-flex align-items-center gap-3 mb-4">
             ...
             <div className="w-100">
               <div className="placeholder col-3 mb-1 bg-dark rounded"></div><br/>
               <div className="placeholder col-4 bg-secondary rounded"></div>
             </div>
          </div>
          <div className="placeholder w-100 rounded mb-4" style={{ height: "400px", backgroundColor: "#e2e8f0" }}></div>
          <div className="mt-4">
  ...
  // Graceful fallback for author while the background fetch is running
  const authorName =
  ...
  return (
    <article className="pb-5 mb-5 mt-5">
      <div className="article-container">
        ...
        <div 
  className="text-start article-body-pro"
            ...
            <div className="d-flex flex-column gap-4">
              {comments.length === 0 ? (
                ...
                comments.map((comment) => {
                  const commenterName =
                    comment.user?.username || comment.user || "Anonymous User";
          ...
          <div className="mt-5 pt-5 border-top border-light">
            ...
            <div className="row g-4">
              {relatedPosts.map((relatedPost) => {
                const relatedPostImage = optimizeCloudinaryUrl(relatedPost.image);
                return (
                  <div key={relatedPost.id} className="col-12 col-md-4">
                    <div
                      ...
                      onClick={() => {
                        ...
                        window.scrollTo({ top: 0, behavior: 'smooth' }); // Scroll to top when clicking a new article
                      }}
                    ...
                    >
                      ...
                      <div className="card-body">
                        <h6 className="card-title fw-bold text-dark text-truncate mb-1">{relatedPost.title}</h6>
                        <small className="text-muted">{new Date(relatedPost.created_at).toLocaleDateString()}</small>
                      </div>
                    </div>
                  </div>
                );
